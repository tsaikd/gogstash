...
func TestDecode(t *testing.T) {
	...
	msgChan := make(chan logevent.LogEvent, 2)
	emptyTags := []string{}
	...
	// message with records array must be splitted to multiples events
	ok, err = codec.Decode(ctx, []byte(`{"records":[{"message":"hello1"},{"message":"hello2"}]}`), nil, emptyTags, msgChan)
	require.NoError(err)
	assert.True(ok)
	require.Len(msgChan, 2)
	event = <-msgChan
